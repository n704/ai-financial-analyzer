...
- **Default market source** (open question #7): `stooq` in `dev`/`scaled` (keyless EOD, split-adjusted), `fixture` in `test`/`offline`. Observed 2026-09: stooq answers non-browser clients with a JavaScript proof-of-work page on every request; the adapter detects it and raises `MarketDataUnavailable` naming the condition (the job fails readably, never a 500), and does not attempt to solve it. Live stooq tests sit behind `LIVE_MARKET_DATA=1`, outside CI.
