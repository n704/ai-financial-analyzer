...
**P6 (market forecasting) is implemented through the API (P6.1–P6.7, P6.9); the UI (P6.8) waits for the UI layer that lands with P2.** `MarketDataProvider`/`ForecastProvider` protocols + `fixture`/`stooq` and `naive`/`fake`/`timesfm` adapters, the license gate and boot checks in the factory, all forecast math in `app/domain/forecast.py`, the global `price_series` cache + user-scoped `forecasts` tables, the queued job (`asyncio.to_thread` in-process, arq when scaled), `/api/v1/forecasts` (+ `/events`, `/narrate`), and the hermetic forecast eval (`tests/evals/forecast/`). Default profiles ship `forecast.enabled: false`; `provider: naive` runs the whole flow with no extra dependencies, `provider: timesfm` needs `make install-forecast`. See ARCHITECTURE.md §4.4 "Implementation notes" for the decisions that refine the spec.
...
A **production-ready, multi-user** RAG web app: users upload financial report PDFs (10-Ks, 10-Qs, earnings releases); the system extracts metrics and generates insight summaries on ingestion, answers questions with page-level citations, compares reports (period-over-period or cross-company), and — from P6 — forecasts the price series of a report's ticker zero-shot with [TimesFM](https://github.com/google-research/timesfm), always shipped with uncertainty bands and a measured backtest scorecard.

**Design axis #1 — everything provider- and infra-shaped is configurable.** The LLM, embedding model, vector store, forecasting model, market-data source, database, cache, queue, and SSE event bus are all chosen in `config.yaml`; application code depends only on interfaces (`LLMProvider` / `EmbeddingProvider` / `VectorStore` / `ForecastProvider` / `MarketDataProvider` / `Cache` / `TaskQueue` / `EventBus`). The **default is zero-dependency**: free-tier Gemini (`gemini-2.5-flash` + `gemini-embedding-001`), **SQLite**, **in-memory cache/queue/events**, local-disk storage — the whole app runs single-process with no external services (`config/dev.yaml`). Scaling to Postgres + Redis + a separate worker is `config/scaled.yaml` — a config change, not a code change. Never hard-code a provider or backend name in core code — see the constraints below.
...
- **Never let the model do arithmetic for comparisons or forecasts** — deltas, growth rates, forecast transforms, and backtest metrics (MASE/sMAPE/pinball/coverage) are computed in Python (`app/domain/`); the model only narrates a table it is handed read-only.
- **Forecasts are measurements, not predictions to be trusted** (P6) — no forecast leaves the server without its 10–90% band and its rolling-origin backtest against seasonal-naive. When the model doesn't beat the baseline, the response and the UI say so (`skill: "no better than naive"`). Equity closes are near-random-walk; reporting weak skill is a passing outcome, hiding it is a defect. No buy/sell/hold, ever.
- **Forecast weights carry a license gate** — TimesFM ≤2.5 weights are Apache-2.0 (default: `google/timesfm-2.5-200m-pytorch`); TimesFM 3.0 weights are non-commercial/non-production and refuse to load without `forecast.weights_license_ack: true`. Unknown checkpoints deny by default. Never import `timesfm`/`torch` or a market-data client outside `app/providers/forecast/*` and `app/providers/marketdata/*`; forecast inference always runs through `TaskQueue`, never inline in a request.
...
Build in order (PLAN.md): **P0 bootstrap ✅ → P1 config/providers/auth → P2 ingest & analyze → P3 RAG Q&A → P4 comparison → P5 hardening → P6 market forecasting (TimesFM).** Each task has an explicit "done when" check in PLAN.md. P6 depends only on the P1 seam and `documents.ticker` from P2.5, so it can be built in parallel with P4/P5.
