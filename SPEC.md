...
7. **Default market-data source** — `stooq` is keyless and EOD-only (least friction); `yfinance` is richer but unofficial and rate-limits hard; Alpha Vantage / Tiingo need keys and carry redistribution terms. **Decided in P6:** `stooq` for the `dev`/`scaled` profiles, `fixture` for `test`/`offline`; the interface makes it swappable, and `fixture` keeps CI hermetic regardless. **Caveat found while testing (2026-09):** stooq now serves a JavaScript proof-of-work challenge to non-browser clients, so the keyless source may be unreachable from unattended deployments — the adapter reports that condition by name rather than working around it; a keyed vendor adapter (Tiingo / Alpha Vantage) is the practical next step.
