...
7. **Default market-data source** — `stooq` is keyless and EOD-only (least friction); `yfinance` is richer but unofficial and rate-limits hard; Alpha Vantage / Tiingo need keys and carry redistribution terms. **Decided in P6:** `stooq` for the `dev`/`scaled` profiles, `fixture` for `test`/`offline`; the interface makes it swappable, and `fixture` keeps CI hermetic regardless.
